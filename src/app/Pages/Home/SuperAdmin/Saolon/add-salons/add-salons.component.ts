...
export class AddSalonsComponent implements OnInit {
  ...
  postVendor() {
    ...
    if (this.vendorDetailPatch) {
      debugger
      let payload = {
        membershipRecordId : this.vendorDetailPatch.membershipRecordId ,
...
debugger
      let payload1 = {
        // membershipRecordId : localStorage.getItem('membershipRecordId'),
        email: this.form.value.email,
        firstName: this.form.value.firstName,
        lastName: this.form.value.lastName,
        gender: this.form.value.gender,
      ...
      this.contentService.addVendor(payload1).subscribe(response => {
        if (response.isSuccess) {
          ...
          this.getItemById();
          this.ids = this.getItemById();
          // this.upidetailId =this.upidetail.upidetailId
          this.fileChangeEvent();
          this.fileChangeEvents();
          this.fileQrChangeEvents();
          this.toasterService.success(response.messages);
  ...
  handleQrFileInput(event: any) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      reader.onload = () => {
        ...
        this.urls.push(this.imageDataUrl);
      };
    }
  }
